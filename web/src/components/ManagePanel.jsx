...
export default function ManagePanel({
  ...
  return (
    <div className={className.trim()}>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-x-5 gap-y-4 bg-base-200 p-4">
        <div className="min-w-[12rem] flex-1"><label className="field-label" htmlFor="cust-name">New client</label><div className="flex items-center gap-2"><input id="cust-name" className="input input-sm w-full" placeholder="Northwind" value={customer.name} onChange={(e) => setCustomer({ ...customer, name: e.target.value })} /><ImagePicker value={customer.imageUrl} name={customer.name} label="Client image" onChange={(imageUrl) => setCustomer({ ...customer, imageUrl })} /></div></div>
        ...
        <button type="button" className="btn btn-primary btn-sm" disabled={busy || !customer.name.trim()} onClick={addCustomer}><Icon name="plus" size={15} />{busy ? "Adding…" : "Add client"}</button>
      </div>

      <div className="mt-3 flex flex-wrap items-end gap-x-5 gap-y-4 bg-base-200 p-4">
        <div className="min-w-[12rem] flex-1"><label className="field-label" htmlFor="proj-name">New project</label><div className="flex items-center gap-2"><input id="proj-name" className="input input-sm w-full" placeholder="Brand refresh" value={project.name} onChange={(e) => setProject({ ...project, name: e.target.value })} /><ImagePicker value={project.imageUrl} name={project.name} label="Project image" onChange={(imageUrl) => setProject({ ...project, imageUrl })} /></div></div>
        <div><label className="field-label" htmlFor="proj-customer">Client</label><select id="proj-customer" className="select select-sm" value={project.customerId} onChange={(e) => setProject({ ...project, customerId: e.target.value })}>{customers.length === 0 ? <option value="">add a client first</option> : null}{customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        ...
        <div className="min-w-[14rem] flex-1"><label className="field-label" htmlFor="proj-default-tasks">Default tasks</label><input id="proj-default-tasks" className="input input-sm w-full" value={project.defaultTasks} onChange={(e) => setProject({ ...project, defaultTasks: e.target.value })} placeholder="General, Meeting" /><p className="mt-1 text-xs ink-muted">Comma-separated; leave blank for none.</p></div>
        <button type="button" className="btn btn-primary btn-sm" disabled={busy || !project.name.trim() || !project.customerId} onClick={addProject}><Icon name="plus" size={15} />{busy ? "Adding…" : "Add project"}</button>
      </div>

      <div className="mt-3 flex flex-wrap items-end gap-x-5 gap-y-4 bg-base-200 p-4">
        <div className="min-w-[12rem] flex-1"><label className="field-label" htmlFor="act-name">New activity</label><div className="flex items-center gap-2"><input id="act-name" className="input input-sm w-full" placeholder="Code review" value={activity.name} onChange={(e) => setActivity({ ...activity, name: e.target.value })} /><ImagePicker value={activity.imageUrl} name={activity.name} emoji={activity.emoji} label="Activity image" onChange={(imageUrl) => setActivity({ ...activity, imageUrl })} /></div><input className="input input-xs mt-2 w-16" aria-label="New activity emoji" placeholder="Emoji" value={activity.emoji} onChange={(e) => setActivity({ ...activity, emoji: e.target.value })} /></div>
        <p className="text-sm ink-muted">Activities are shared: Development, Meeting, Design…</p>
        <button type="button" className="btn btn-primary btn-sm" disabled={busy || !activity.name.trim()} onClick={addActivity}><Icon name="plus" size={15} />{busy ? "Adding…" : "Add activity"}</button>
      </div>

      <div className="mt-10"><h3 className="mb-2 text-base font-semibold">Clients</h3><p className="ink-muted mb-3 text-sm">The client's currency is used for every rate and billed amount on their projects and entries.</p><div className="overflow-x-auto"><table className="table table-sm"><thead><ListHead><th>Name</th><th>Currency</th><th>Rate</th><th /></ListHead></thead><tbody>{customers.length ? customers.map((customerItem) => <CustomerRow key={customerItem.id} customer={customerItem} busy={busy} onSave={(patch) => onSaveCustomer(customerItem.id, patch)} onArchive={() => onArchiveCustomer(customerItem.id)} />) : <tr><td colSpan="4" className="py-6 text-sm ink-muted">No clients yet. Add your first client above to define a currency and hourly rate.</td></tr>}</tbody></table></div></div>
      <div className="mt-8"><h3 className="mb-2 text-base font-semibold">Projects</h3><div className="overflow-x-auto"><table className="table table-sm"><thead><ListHead><th>Name</th><th>Client</th><th>Rate</th><th>Bills</th><th /></ListHead></thead><tbody>{projects.length ? projects.map((projectItem) => <ProjectRow key={projectItem.id} project={projectItem} customers={customers} busy={busy} onSave={(patch) => onSaveProject(projectItem.id, patch)} onArchive={() => onArchiveProject(projectItem.id)} />) : <tr><td colSpan="5" className="py-6 text-sm ink-muted">No projects yet. Add a client first, then create the project you will track time against.</td></tr>}</tbody></table></div></div>
      <div className="mt-8"><h3 className="mb-2 text-base font-semibold">Activities</h3><div className="overflow-x-auto"><table className="table table-sm"><tbody>{activities.length ? activities.map((activityItem) => <ActivityRow key={activityItem.id} activity={activityItem} busy={busy} onSave={(patch) => onSaveActivity(activityItem.id, patch)} onArchive={() => onArchiveActivity(activityItem.id)} />) : <tr><td className="py-6 text-sm ink-muted">No activities yet. Add a shared activity such as Development, Design, or Meeting.</td></tr>}</tbody></table></div></div>
