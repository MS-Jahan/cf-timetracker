...
- `PATCH /api/entries/:id` edits an entry's `customerId`, `projectId`, `activityId`, `description`, `tags`, `startTime`, `endTime`, and `hourlyRate`; `startTime`/`endTime` are epoch milliseconds, and the worker recalculates duration and cost from the edited time range and rate. Running entries are editable in place: a patch without `endTime` updates fields and keeps the clock running; a patch with `endTime` acts as a manual stop (`is_running` flips to 0 under the same `WHERE ... AND is_running = 1` + changes-check discipline as `stopTimer`). `DELETE /api/entries/:id` hard-deletes a closed entry; the running row (the live timer) must be stopped first and answers 409.
...
- **Historical entries:** closed rows are editable through `PATCH /api/entries/:id`; duration and cost are derived values, never independently patched. The UI exposes the editor from each ledger row (running rows edit in place, with an optional end time that stops the timer), plus duplicate/start-again/delete row actions on the timesheet.
