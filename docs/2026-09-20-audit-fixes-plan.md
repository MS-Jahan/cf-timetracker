...
- [x] **T16 - Logo + banner.** Brand assets added: originals in `docs/assets/` (logo, dark variant, GitHub banner); web-ready derivatives in `web/public/` (`logo.png` 768w for print header, `logo-mark.png` transparent mark for sidebar + splash, `favicon.png`, `logo-dark.png`). Favicon + tab title set in `index.html`. Print header shows the logo instead of the dashed placeholder. README opens with the GitHub banner (per owner: banner replaces the logo there). Em dashes removed from all project text (README, docs, code comments, UI strings).
